...
select distinct
51 AP,
(select top 1 cnes from RECEPCAO.dbo.PEP_Parametros) CNES,
...
INNER JOIN PEPMOVIMENTOS B ON A.MOV_ID = B.CODIGO
where c.idpep=B.CODPAC AND A.registro_fundoscopia IS NOT NULL),'')DT_FUNDOSCOPIA_ESQ,
isnull((select top 1  case when A.registro_fundoscopia = 'S' then '6'
else '' end FUNDOSCOPIA_ESQ from protocolo_hiperdia A 
INNER JOIN PEPMOVIMENTOS B ON A.MOV_ID = B.CODIGO
...
isnull((select top 1 CONVERT (VARCHAR (10), b.datafinaliza, 103)  from protocolo_hiperdia A 
INNER JOIN PEPMOVIMENTOS B ON A.MOV_ID = B.CODIGO
where c.idpep=B.CODPAC AND A.registro_fundoscopia IS NOT NULL),'')DT_FUNDOSCOPIA_DIR,
isnull((select top 1 CONVERT (VARCHAR (10), b.datafinaliza, 103)  from protocolo_hiperdia A 
INNER JOIN PEPMOVIMENTOS B ON A.MOV_ID = B.CODIGO
...
when  A.AVALIACAO_PE_DIABETICO LIKE '5' then '4'	
when  A.AVALIACAO_PE_DIABETICO LIKE '6' then '5'		
else '' end CATEG_RISC_MMII_DIR from protocolo_hiperdia A 
INNER JOIN PEPMOVIMENTOS B ON A.MOV_ID = B.CODIGO
	...
	ISNULL ((select top 1 convert(varchar(10), data, 103) from recepcao.[exames].[Trigliceridios] where codpac = c.idpep order by data desc),'') DT_TRIG,
	  ...
	  WHERE B.CODPAC = C.IDPEP ORDER BY A.ID DESC),'') PAD,
	   ...
	   when [recepcao].[dicionario].[Fr_TrazPressaoDiastolicaPac](c.idpep) >= 110 then '4'
		...
		isnull((select top 1 
				case 
					when convert(numeric(5,1), rtrim(ltrim(replace(replace(case when aa.hemoglobina = '' then '0' else aa.hemoglobina end,',','.'),'%',''))) )  <= 7.0 then '1' 
					when convert(numeric(5,1), rtrim(ltrim(replace(replace(case when aa.hemoglobina = '' then '0' else aa.hemoglobina end,',','.'),'%',''))) ) >= 7.1
					and  convert(numeric(5,1), rtrim(ltrim(replace(replace(case when aa.hemoglobina = '' then '0' else aa.hemoglobina end,',','.'),'%',''))) ) <=8.0 then '2' 
					when convert(numeric(5,1), rtrim(ltrim(replace(replace(case when aa.hemoglobina = '' then '0' else aa.hemoglobina end,',','.'),'%',''))) ) >= 8
					and  convert(numeric(5,1), rtrim(ltrim(replace(replace(case when aa.hemoglobina = '' then '0' else aa.hemoglobina end,',','.'),'%',''))) ) <=10 then '3' 
					when convert(numeric(5,1), rtrim(ltrim(replace(replace(case when aa.hemoglobina = '' then '0' else aa.hemoglobina end,',','.'),'%',''))) ) >= 10.1
					and  convert(numeric(5,1), rtrim(ltrim(replace(replace(case when aa.hemoglobina = '' then '0' else aa.hemoglobina end,',','.'),'%',''))) ) <=11.9 then '4' 
					when convert(numeric(5,1), rtrim(ltrim(replace(replace(case when aa.hemoglobina = '' then '0' else aa.hemoglobina end,',','.'),'%',''))) )  >= 12.0 then '5' 
					else '' 
				end 
				from recepcao.[exames].[hba1c_hemoglobina_glicada] aa where aa.codpac = c.idpep  and  isnumeric(aa.hemoglobina)=1 order by aa.data desc
				),'') HBA1C,
...
INNER join recepcao.dbo.pessoa_grupo_de_pessoas d on c.IDPep=d.cod_pac and d.grupo_de_pessoas_id=7 and d.ATIVO='S'
INNER join recepcao.dbo.pessoa_grupo_de_pessoas e on c.IDPep=e.cod_pac and e.grupo_de_pessoas_id=6 and e.ATIVO='S'
