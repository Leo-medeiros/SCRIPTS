...
/*Slide 11 A 14 - FALTA A VARIAVEL D9 (VER COM SIDNEY)*/
BEGIN
...
/* Mulheres em idade fertil entre 10 e 49 anos cadastradas*/
USE recepcao;
SELECT COUNT(*) FROM indicadores.vw_pep_siab a
INNER JOIN recepcao.dbo.pacientes b ON a.idpep = b.codigo
WHERE a.sexo = 'F'
AND a.idade BETWEEN '10' AND '49'
AND b.cod_familia IS NOT NULL
AND b.datacadastro IS NULL
AND CONVERT (VARCHAR(10), b.datacadastro,103) <= '31/12/2015' 

/* slide 16 */


/* slide 17 */
/* Nº de casos notificados 1 º Semestre 2015*/

SET DATEFORMAT YMD
SELECT W.* from recepcao.dbo.prontuario_gestante x inner join recepcao.dbo.PepMovimentos y on x.id_ = y.codigo 
inner join recepcao.[dbo].Prontuario_Gestante_Teste_Rapido w 
on x.id_Pront_Gestante_Teste_Rapido = w.id_Pront_Gestante_Teste_Rapido
WHERE w.sifilis_result = 'P'
AND w.sifilis_data BETWEEN '2015-05-01 00:00:00' and '2015-06-30 23:59:59'

/* Nº de casos notificados 2 º Semestre 2015*/
...
on x.id_Pront_Gestante_Teste_Rapido = w.id_Pront_Gestante_Teste_Rapido
WHERE w.sifilis_result = 'P'
AND w.sifilis_data BETWEEN '2015-07-01 00:00:00' and '2015-12-31 23:59:59'

/* Nº de casos notificados 1 º Semestre 2016*/
...
on x.id_Pront_Gestante_Teste_Rapido = w.id_Pront_Gestante_Teste_Rapido
WHERE w.sifilis_result = 'P'
AND w.sifilis_data BETWEEN '2016-01-01 00:00:00' and '2016-06-30 23:59:59'
...
SELECT * FROM DBO.PESSOA_GRUPO_DE_PESSOAS WHERE YEAR(DATA_INICIAL) = @COMP AND GRUPO_DE_PESSOAS_ID = 3
...
LEFT JOIN SIABFACIL.FAMILIA B ON A.FAMILIA_ID = B.ID
LEFT JOIN CNES.LFCES037 C ON B.COD_AREA = C.COD_AREA
WHERE A.FAMILIA_ID IS NOT NULL
AND B.COD_AREA IS NOT NULL') WHERE grupo_de_pessoas_id = 21  and
(year(data_entrada) = 2015 or year(data_saida) = 2015)
group by equipe
order by equipe
...
LEFT JOIN CNES.LFCES037 C ON B.COD_AREA = C.COD_AREA
--WHERE A.FAMILIA_ID IS NOT NULL
WHERE B.COD_AREA IS NOT NULL') WHERE grupo_de_pessoas_id = 21  and
(year(data_entrada) = 2016 or year(data_saida) = 2016) and (month(data_entrada) in (1,2,3,4,5,6) or month(data_saida) in (1,2,3,4,5,6))
group by equipe
order by equipe
...
where codproc = '0301010099'  and year(dtatende) = 2015
group by nomeequipe
order by nomeequipe
...
inner join dbo.pepmovimentos c on a.id_pepmovimentos = c.codigo
inner join indicadores.vw_pep_siab d on d.idpep = c.codpac 
where codproc = '0301010099' and year(dtatende) = 2016 and month(dtatende) in (1,2,3,4,5,6)
group by nomeequipe
order by nomeequipe
...
/* slide 29 */
select count(1)  from [dbo].[PepMovimentos_Soli_Procedimento] a 
inner join dbo.procedimentos b on a.id_exame = b.id
inner join dbo.pepmovimentos c on a.id_pepmovimentos = c.codigo
inner join indicadores.vw_pep_siab d on d.idpep = c.codpac 
where codproc = '0101010044' and year(dtatende) = 2016 
/* slide 30 */
SELECT * FROM OPENQUERY (SIAB,'
SELECT COUNT(*) FROM SIABFACIL.PESSOA  WHERE SITUACAO_RUA = TRUE AND EXTRACT (YEAR FROM DATA_ATIVACAO) = 2015')

--pep
select count(distinct a.codpac) 
from RECEPCAO.dbo.PepMovimentos a 
inner join RECEPCAO.Indicadores.vw_pep_siab b on a.codpac = b.idpep
inner join OPENQUERY (SIAB,'SELECT id FROM SIABFACIL.PESSOA  WHERE SITUACAO_RUA = TRUE AND EXTRACT (YEAR FROM DATA_ATIVACAO) <= 2015') c on b.idsiab = c.id
where year(dtatende) =2015

-- siab
select * from openquery(siab, ' SELECT COUNT(DISTINCT PESSOA_ID) FROM SIABFACIL.ATENDIMENTO a inner join 
 SIABFACIL.PESSOA b on a.PESSOA_ID = b.id WHERE EXTRACT (YEAR FROM DATA) = 2015
and SITUACAO_RUA = TRUE AND EXTRACT (YEAR FROM DATA_ATIVACAO) <= 2015')
...
/* slide 39 */
--vds
SELECT count(*) from recepcao.dbo.pepmovimentos a 
INNER JOIN MEDICOS B on a.codmed = b.codigo 
 where CBO1 IN ('223810', '223710', '223605','223905')
 AND YEAR (DTATENDE) = 2015
 AND MOVIMENTO = 3

 --atividade_grupo
 SELECT COUNT(*) FROM atividade_grupo a
INNER JOIN  atividade_prof_grupo B ON A.ID = B.id_atividade
INNER JOIN ACESSO.DBO.USUARIOS D ON B.ID_PROF = D.CODIGO
INNER JOIN MEDICOS C on D.ID_MEDICO = C.codigo 
 where CBO1 IN ('223810', '223710', '223605','223905')
 AND YEAR (DATA_ATIVIDADE) = 2015
/* slide 40 */
...
BEGIN
    ...
    --SELECT * FROM CBO
	 ...
	 SELECT COUNT(*) FROM atividade_grupo a
INNER JOIN  atividade_prof_grupo B ON A.ID = B.id_atividade
INNER JOIN ACESSO.DBO.USUARIOS C on B.ID_PROF = C.codigo 
 AND YEAR (DATA_ATIVIDADE) = 2015
AND C.EQUIPE = @COD_EQU
...
INNER JOIN MEDICOS C on D.ID_MEDICO = C.codigo 
 ...
 AND C.EQUIPE = @COD_EQU
	...
	SELECT @ficha_a_0_1 = count(1) FROM Accountability.vw_pessoa where data_ativacao <= '2016-05-31 23:59:59' and dbo.CalcularIdadeAtende(data_nascimento, '2016-05-31 23:59:59') between 0 and 1 and cod_equipe = @COD_EQU
    SELECT @ficha_b_0_1 = count(distinct a.id) FROM Accountability.vw_pessoa a inner join indicadores.vw_pep_siab b on a.id = b.idsiab inner join dbo.pepmovimentos c on b.idpep = c.codpac where data_ativacao <= '2016-05-31 23:59:59' and dtatende <= '2016-05-31 23:59:59' and dbo.CalcularIdadeAtende(a.data_nascimento, '2016-05-31 23:59:59') between 0 and 1 and cod_equipe = @COD_EQU
	
	--Nº de crianças de 1 a 2 ano
	declare @ficha_a_1_2 int
	declare @ficha_b_1_2 int 
	SELECT @ficha_a_1_2 = count(1) FROM Accountability.vw_pessoa where data_ativacao  <= '2016-05-31 23:59:59' and dbo.CalcularIdadeAtende(data_nascimento, '2016-05-31 23:59:59') between 1 and 2 and cod_equipe = @COD_EQU
	...
	union all 
	select 'Gestante nao foi feito' titulo, 0, 0, 0, 0
	...
	declare @cartao_carioca int
	declare @nasf_12_19 int
	...
	union all
	select 'Das gestantes BF  acompanhadas, quantas foram matriciados pelo NASF ² (nº absoluto)', 0
    FETCH NEXT FROM cursor_objects INTO @COD_EQU, @NOME_EQU
END
